...
class PAL5Classsifier(JointEncodingRetrievalClassifier):
    ...
    def compute_features(self):
        ...
        encoding_powers, encoding_events = compute_powers(encoding_events, self.channels, self.pairs, self.start_time,
                                                          self.end_time, self.buffer_time, self.freqs, True)
        retrieval_powers,retrieval_events = compute_powers(retrieval_events, self.channels, self.pairs,
                                                           self.retrieval_start_time, self.retrieval_end_time,
                                                           self.retrieval_buf, self.freqs, True)
