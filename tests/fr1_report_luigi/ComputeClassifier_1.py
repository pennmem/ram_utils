...
class ComputeClassifier_1(RamTaskL):
    params = None
    pow_mat = None
    lr_classifier = None
    xval_output = dict()  # ModelOutput per session; xval_output[-1] is across all sessions
    perm_AUCs = None
    pvalue = None
    _events = None
    ...
    def define_outputs(self):
        ...
        self.add_file_resource('pvalue')

    def requires(self):
        yield ComputeFR1Powers_1(pipeline=self.pipeline)
    ...
    def input_hashsum(self):
        ...
        fr1_event_files = sorted(
            list(json_reader.aggregate_values('all_events', subject=subj_code, montage=montage, experiment='FR1')))
    ...
    def events(self):
        ...
        self._events = self.get_passed_object(self.pipeline.task + '_events_compute_powers')
...
class ComputeJointClassifier_1(RamTaskL):
    params = None
    pow_mat = None
    lr_classifier = None
    xval_output = dict()  # ModelOutput per session; xval_output[-1] is across all sessions
    ...
    def define_outputs(self):
        ...
        subject = self.pipeline.subject
        self.add_file_resource('joint_lr_classifier')
        self.add_file_resource('joint_xval_output')
        self.add_file_resource('joint_perm_AUCs')
        self.add_file_resource('joint_pvalue')
        ...
        for sess_idx, sess in enumerate(sessions):
            ...
            if events is not None:
                ...
                auc_retrieval[sess_idx] = self.get_auc(
                    classifier=self.lr_classifier, features=self.pow_mat, recalls=recalls,
