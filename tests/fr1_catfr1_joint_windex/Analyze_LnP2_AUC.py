...
def quadratic_feature_indices(i,j):
    if i/8 == j/8:
        return []
    freq_i = i%8
    freq_j = j%8
    if freq_i==freq_j:
        ...
        i_fj = i - freq_i + freq_j
        j_fi = j - freq_j + freq_i
        return [(i_fj,j),(i,j_fi)] if i<j else [(j,i_fj),(j_fi,i)]

def lift_and_project(insample_pow_mat, insample_recalls, insample_events, outsample_pow_mat, outsample_events):
    ...
    for i in xrange(1,25):
        for j in xrange(i):
            quadratic_features += quadratic_feature_indices(feat_idx_best_to_worst[i],feat_idx_best_to_worst[j])
    quadratic_features_ar = np.empty(len(quadratic_features), dtype=np.object)
    quadratic_features_ar[:] = quadratic_features[:]
    quadratic_features = np.unique(quadratic_features_ar)
    n_qf = len(quadratic_features)

    insample_pow_quad_mat = np.empty(shape=(insample_pow_mat.shape[0],n_qf), dtype=float)
    outsample_pow_quad_mat = np.empty(shape=(outsample_pow_mat.shape[0],n_qf), dtype=float)
    for k,p in enumerate(quadratic_features):
        insample_pow_quad_mat[:,k] = insample_pow_mat[:,p[0]] * insample_pow_mat[:,p[1]]
        outsample_pow_quad_mat[:,k] = outsample_pow_mat[:,p[0]] * outsample_pow_mat[:,p[1]]

    insample_pow_quad_mat = normalize_sessions(insample_pow_quad_mat, insample_events)
    outsample_pow_quad_mat = normalize_sessions(outsample_pow_quad_mat, outsample_events)
    insample_pow_mat = insample_pow_mat[:,feat_idx_best_to_worst[:25]]
    outsample_pow_mat = outsample_pow_mat[:,feat_idx_best_to_worst[:25]]
    insample_pow_mat = np.concatenate((insample_pow_mat,insample_pow_quad_mat), axis=1)
    outsample_pow_mat = np.concatenate((outsample_pow_mat,outsample_pow_quad_mat), axis=1)
    recalled_insample_pow_mat = insample_pow_mat[insample_recalls,:]
    nonrecalled_insample_pow_mat = insample_pow_mat[~insample_recalls,:]
    t,_ = ttest_ind(recalled_insample_pow_mat, nonrecalled_insample_pow_mat, axis=0)
    feat_idx_best_to_worst = np.argsort(-np.abs(t))
    return insample_pow_mat, outsample_pow_mat, feat_idx_best_to_worst


class Analyze_LnP2_AUC(ReportRamTask):
    def __init__(self, params, mark_as_completed=True):
        super(Analyze_LnP2_AUC,self).__init__(mark_as_completed)
        self.params = params
        self.pow_mat = None
    ...
    def run(self):
        ...
        max_feat_num = 101 #min(101, self.pow_mat.shape[1])
        ...
        for heldout_sess in sessions:
            ...
            insample_pow_mat = self.pow_mat[insample_mask,:]
            insample_recalls = recalls[insample_mask]
            insample_events = events[insample_mask]

            outsample_mask = ~insample_mask
            outsample_pow_mat = self.pow_mat[outsample_mask,:]
            outsample_events = events[outsample_mask]

            insample_pow_mat, outsample_pow_mat, feat_idx_best_to_worst = lift_and_project(insample_pow_mat, insample_recalls, insample_events, outsample_pow_mat, outsample_events)
        ...
        joblib.dump(self.outsample_probs, self.get_path_to_resource_in_workspace('fs_lnp2_outsample_probs.pkl'))
        joblib.dump(self.AUCs, self.get_path_to_resource_in_workspace('fs_lnp2_AUCs.pkl'))


    def restore(self):
        self.outsample_probs = joblib.load(self.get_path_to_resource_in_workspace('fs_lnp2_outsample_probs.pkl'))
        self.AUCs = joblib.load(self.get_path_to_resource_in_workspace('fs_lnp2_AUCs.pkl'))
