...
class RegionFrequencyAnalysis(object):
    ...
    def run(self, ps_table, regions, areas):
        ...
        freq_mean = burst_frequency_groups['prob_diff'].mean().values
        ...
        self.frequency_plot = PlotData(x=np.arange(1,len(self.freqs)+1), y=freq_mean, yerr=freq_sem, ylim=ylim, xlabel='Burst Frequency (Hz)', x_tick_labels=self.freqs)
