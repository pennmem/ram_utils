...
class ComputeFR4Powers(ReportRamTask):
    def __init__(self, params, mark_as_completed=True):
        super(ComputeFR4Powers,self).__init__(mark_as_completed)
        self.params = params
        self.pow_mat = None
    ...
    def initialize(self):
        task_prefix = 'cat' if self.pipeline.task == 'RAM_CatFR4' else ''
    ...
    def restore(self):
        ...
        self.pow_mat = joblib.load(self.get_path_to_resource_in_workspace(subject + '-' + task + '-fr4_pow_mat.pkl'))
        self.samplerate = joblib.load(self.get_path_to_resource_in_workspace(subject + '-samplerate.pkl'))

        self.pass_object('fr4_pow_mat', self.pow_mat)
    ...
    def run(self):
        ...
        joblib.dump(self.pow_mat, self.get_path_to_resource_in_workspace(subject + '-' + task + '-fr4_pow_mat.pkl'))
