...
def construct_stim_item_mask(events):
    ...
    for i in xrange(len(events)):
        if word_event_mask[i]:
            stim_item_mask[j] = (events[i+1].isStim==1 or events[i+2].isStim==1 or events[i+3].isStim==1)
            j += 1
...
class FR3EventPreparation(RamTask):
    ...
    def run(self):
        ...
        events = Events(get_events(subject=self.pipeline.subject, task=task3, path_prefix=self.pipeline.mount_point))

        events = correct_eegfile_field(events)
        ev_order = np.argsort(events, order=('session','list','mstime'))
        events = events[ev_order]

        all_events = self.attach_raw_bin_wrappers(events)
