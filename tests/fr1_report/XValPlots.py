...
class XValPlots(RamTask):
    ...
    def run(self):
        ...
        pd = PlotData(x=x, y=y, yerr=yerr, xhline_pos=0.5, ylim=(0.0, 1.0))
        pd_features = PlotData(x=x, y=y_features,yerr=y_features_err )
        ...
        panel_plot = PanelPlot(i_max=1, j_max=2, title='', y_axis_title='AUC')
        ...
        for plot_panel_index, pd in pd_dict.iteritems():
            ...
            panel_plot.add_plot_data(plot_panel_index[0], plot_panel_index[1], x=pd.x, y=pd.y, yerr=pd.yerr,ylim=pd.ylim,xhline_pos=pd.xhline_pos)
                # ,
        ...
        plot_out_fname = 'plot_'+self.pipeline.subject+'.pdf'
        plot.savefig(plot_out_fname, dpi=300, bboxinches='tight')
