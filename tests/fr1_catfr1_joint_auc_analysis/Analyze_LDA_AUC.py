...
class Analyze_LDA_AUC(ReportRamTask):
    def __init__(self, params, mark_as_completed=True):
        super(Analyze_LDA_AUC,self).__init__(mark_as_completed)
        self.params = params
        self.pow_mat = None
    ...
    def run(self):
        ...
        max_feat_num = min(101, self.pow_mat.shape[1])
        ...
        for heldout_sess in sessions:
            ...
            insample_pow_mat = self.pow_mat[insample_mask,:]
            insample_pow_mat = self.pca.fit_transform(insample_pow_mat, insample_recalls)
        ...
        joblib.dump(self.outsample_probs, self.get_path_to_resource_in_workspace('fs_spca_outsample_probs.pkl'))
        joblib.dump(self.AUCs, self.get_path_to_resource_in_workspace('fs_spca_AUCs.pkl'))


    def restore(self):
        self.outsample_probs = joblib.load(self.get_path_to_resource_in_workspace('fs_spca_outsample_probs.pkl'))
        self.AUCs = joblib.load(self.get_path_to_resource_in_workspace('fs_spca_AUCs.pkl'))
