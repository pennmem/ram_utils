...
def choose_location(dataset_loc_0, loc_name_0, dataset_loc_1, loc_name_1, bounds, sham_delta_classifier_vector=None,
    ...
    # loc_info = [collections.OrderedDict(), collections.OrderedDict()]
    loc_info = collections.OrderedDict()
    loc_names = [loc_name_0, loc_name_1]

    for i, loc_name in enumerate(loc_names):
        x_max_loc[i], y_max_loc[i], sem_list[i], SNR[i] = find_max(xp_loc[i], yp_loc[i], model,
    ...
    # decision['best_location_index'] = 0
    ...
    print 'champion_mean=', champion_mean
    print 'champion_sem=', champion_sem
