...
class GeneratePlots(ReportRamTask):
    ...
    def run(self):
        ...
        # This subset of events is used in the EEG plot
        all_events = self.get_passed_object('all_events')
        post_stim_events = all_events[all_events.type=='STIM_OFF']
        
        # This subset of events is used in biomarkers
        task_events = self.get_passed_object(task+'_events')
        stim_off_events = self.get_passed_object('stim_off_events')
        biomarker_events = np.concatenate([events,stim_off_events]).view(np.recarray)
        pre_stim_biomarker_events = biomarker_events[(biomarker_events.type == 'WORD')]
        post_stim_biomarker_events = biomarker_events[(biomarker_events.type == 'STIM_OFF')]
        pre_stim_probs = self.get_passed_object('pre_stim_probs')
        post_stim_probs = self.get_passed_object('post_stim_probs')

        # Post stim eeg by session
        post_stim_eeg = self.get_passed_object('post_stim_eeg')

        sessions = np.unique(events.session)
        serial_positions = np.arange(1, 13)
        for i, session_summary in enumerate(session_summaries):
            # Session-specific classifier performance
            panel_plot = PanelPlot(xfigsize=15, yfigsize=7.5, i_max=1, j_max=2, labelsize=16, wspace=5.0)
            pd1 = PlotData(x=xval_output[session_summary.session].fpr, y=xval_output[session_summary.session].tpr, xlim=[0.0, 1.0], ylim=[0.0, 1.0],
                        xlabel='False Alarm Rate\n(a)', ylabel='Hit Rate', xlabel_fontsize=20, ylabel_fontsize=20,
                        levelline=((0.0, 1.0), (0.0, 1.0)), color='k', markersize=1.0)
            pc_diff_from_mean = (
            xval_output[session_summary.session].low_pc_diff_from_mean, xval_output[session_summary.session].mid_pc_diff_from_mean, xval_output[session_summary.session].high_pc_diff_from_mean)
            ylim = np.max(np.abs(pc_diff_from_mean)) + 5.0
            if ylim > 100.0:
                ylim = 100.0
            pd2 = BarPlotData(x=(0, 1, 2), y=pc_diff_from_mean, ylim=[-ylim, ylim],
            ...
            plot = panel_plot.generate_plot()
            plot_out_fname = self.get_path_to_resource_in_workspace(
                'reports/' + self.pipeline.subject + '-roc_and_terc_plot_{}.pdf'.format(i))
            plot.savefig(plot_out_fname, dpi=300, bboxinches='tight')
            session_summary.ROC_AND_TERC_PLOT_FILE = plot_out_fname
            self.pass_object('ROC_AND_TERC_PLOT_FILE_{}'.format(i),plot_out_fname)
            ...
            pd = BarPlotData(x = np.arange(len(hist))-0.25,y=hist,xlabel = 'Pre-stim classifier output',ylabel='',xlabel_fontsize=20,
                            ...
                            ylim=[0, len(pre_stim_probs_session)/2])
            ...
            pd = BarPlotData(x = np.arange(len(hist))-0.25,y=hist,xlabel = 'Post-stim classifier output',ylabel='',xlabel_fontsize=20,
                            x_tick_labels=x_tick_labels,
                            ylim =[0,len(post_stim_probs_session)/2])
            panel_plot.add_plot_data(0,1,plot_data=pd)
            plt = panel_plot.generate_plot()
            
            figname = self.get_path_to_resource_in_workspace('reports/'+self.pipeline.subject+'-biomarker-histograms-{}.pdf'.format(i))
            session_summary.BIOMARKER_HISTOGRAM_PLOT_FILE = figname
            ...
            figname = self.get_path_to_resource_in_workspace(join('reports',self.pipeline.subject+'-post-stim-eeg-{}.pdf'.format(i)))
            session_summary.POST_STIM_EEG_PLOT_FILE = figname
            plt.savefig(figname)
            plt.close()
            ...
            for target in targets:
                ...
                pd1a = PlotData(x=session_summary.prob_stim_recall[target].index.values, y=session_summary.prob_stim_recall[target].values,linestyle='-',label='Stim items')
                pdca.add_plot_data(pd1a)
                pd2a = PlotData(x = session_summary.prob_nostim_recall[target].index.values, y=session_summary.prob_nostim_recall[target].values,linestyle = '--',label='Non-stim Items')
                ...
                pd1b = PlotData(x=serial_positions, y = session_summary.prob_first_stim_recall[target], linestyle = '-', label = 'Stim Items')
                pd2b = PlotData(x=serial_positions, y=session_summary.prob_first_nostim_recall[target], linestyle = '--', label = 'Non-stim Items')
                ...
                n_lists = len(session_summary.n_stims_per_list[target])
                ...
                bpd_1 = BarPlotData(x=np.arange(n_lists), y=session_summary.n_stims_per_list[target],
                                    x_tick_labels=x_tick_labels, title='', alpha=0.3)
                stim_x = np.where(session_summary.is_stim_list[target])[0]
                stim_y = session_summary.n_recalls_per_list[target][session_summary.is_stim_list[target]]
                pd_1 = PlotData(x=stim_x, y=stim_y, ylim=(0, 12),
                                title='', linestyle='', color='red', marker='o', markersize=12)
                plot_data_list = [bpd_1,pd_1]
                if session_summary.is_stim_list[target].all():
                    nostim_x = np.where(session_summary.is_ps_list[target])[0]
                    nostim_y = session_summary.n_recalls_per_list[target][session_summary.is_ps_list[target]]
                    pd_2 = PlotData(x=nostim_x, y=nostim_y, ylim=(0, 12),
                                    title='', linestyle='', color='grey', marker='o', markersize=12)
                    baseline_x = np.where(session_summary.is_baseline_list[target])[0]
                    baseline_y = session_summary.n_recalls_per_list[target][np.array(session_summary.is_baseline_list[target])]
                    ...
                    nostim_x = np.where(session_summary.is_nonstim_list[target])[0]
                    nostim_y = session_summary.n_recalls_per_list[target][session_summary.is_nonstim_list[target]]
                    pd_2 = PlotData(x=nostim_x, y=nostim_y, ylim=(0, 12),
                                    title='', linestyle='', color='blue', marker='o', markersize=12)
                ...
                session_summary.STIM_AND_RECALL_PLOT_FILE[target] = self.get_path_to_resource_in_workspace(
                    'reports/' + task + '-' + subject + '-stim_and_recall_plot_'+ str(session_summary.session) + "-" + session_summary.stimtag[target] + '-' + str(
                        session_summary.frequency[target]) + '.pdf')
                ...
                panel_plot.add_plot_data(0,0,plot_data=pd)
                plot = panel_plot.generate_plot()
                session_summary.PROB_STIM_PLOT_FILE[target] = self.get_path_to_resource_in_workspace('reports/'+subject+'_stim_plot_session_'+ str(session_summary.session) + "-" + session_summary.stimtag[target]+'-'+str(session_summary.frequency[target])+'.pdf')
                plot.savefig(session_summary.PROB_STIM_PLOT_FILE[target],dpi=300,bboxinches='tight')
                ...
                session_tex = replace_template_to_string('fr6_session.tex.tpl',
                             {
                                 ...
                                 '<PERM-P-VALUE>': p_values[session_summary.session],
                                 '<JSTAT-THRESH>': '%2.2f' % xval_output[session_summary.session].jstat_thresh,
