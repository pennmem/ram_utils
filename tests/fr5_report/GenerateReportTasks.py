...
class GeneratePlots(ReportRamTask):
    ...
    def run(self):
        ...
        for session_summary in fr5_session_summaries:
            ...
            pd1a = PlotData(x=serial_positions, y=session_summary.prob_stim_recall,linestyle='-',label='Stim items')
            pdca.add_plot_data(pd1a)
            pd2a = PlotData(x = serial_positions, y=session_summary.prob_nostim_recall,linestyle = '--',label='Non-stim Items')
