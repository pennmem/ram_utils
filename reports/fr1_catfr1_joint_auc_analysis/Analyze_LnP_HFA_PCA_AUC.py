...
def lift_and_project(insample_pow_mat, insample_recalls, insample_events, outsample_pow_mat, outsample_events):
    ...
    insample_pow_quad_mat = np.empty(shape=(insample_pow_mat.shape[0],25*24/2), dtype=float)
    outsample_pow_quad_mat = np.empty(shape=(outsample_pow_mat.shape[0],25*24/2), dtype=float)
    k = 0
    for i in xrange(1,25):
        for j in xrange(i):
            insample_pow_quad_mat[:,k] = insample_pow_mat[:,i] * insample_pow_mat[:,j]
            outsample_pow_quad_mat[:,k] = outsample_pow_mat[:,i] * outsample_pow_mat[:,j]
            k += 1
...
class Analyze_LnP_HFA_PCA_AUC(ReportRamTask):
    def __init__(self, params, mark_as_completed=True):
        super(Analyze_LnP_HFA_PCA_AUC,self).__init__(mark_as_completed)
        self.params = params
        self.pow_mat = None
    ...
    def run(self):
        ...
        self.pow_mat = self.pow_mat[:,np.repeat([False, False, False, False, False, False, True, False], self.pow_mat.shape[1]/8)]
        ...
        for heldout_sess in sessions:
            ...
            insample_pow_mat = self.pow_mat[insample_mask,:]
            insample_pow_mat = self.pca.fit_transform(insample_pow_mat)
            insample_recalls = recalls[insample_mask]
            insample_events = events[insample_mask]

            outsample_mask = ~insample_mask
            outsample_pow_mat = self.pow_mat[outsample_mask,:]
            outsample_pow_mat = self.pca.transform(outsample_pow_mat)
        ...
        joblib.dump(self.outsample_probs, self.get_path_to_resource_in_workspace('fs_lnp_hfa_pca_outsample_probs.pkl'))
        joblib.dump(self.AUCs, self.get_path_to_resource_in_workspace('fs_lnp_hfa_pca_AUCs.pkl'))


    def restore(self):
        self.outsample_probs = joblib.load(self.get_path_to_resource_in_workspace('fs_lnp_hfa_pca_outsample_probs.pkl'))
        self.AUCs = joblib.load(self.get_path_to_resource_in_workspace('fs_lnp_hfa_pca_AUCs.pkl'))
