...
class TestEvents:
    ...
    def test_build_training_data_regression(self, subject, experiment,
        ...
        assert np.array_equal(current_events.recalled,
                              expected_events.recalled)
    ...
    @pytest.mark.parametrize('subject, experiment, params, joint_report, '
                             'sessions', [
                                 ('R1350D', 'FR1', FRParameters,
                                  True, None),  # multi-session FR
                                 # fr and catfr combined
                                 ('R1354E', 'FR1', FRParameters, True, None),
                                 ('R1354E', 'FR1', FRParameters,
                                  False, None),  # FR1 only
                                 ('R1354E', 'catFR1', FRParameters,
                                  False, [0]),  # catFR1 only
                                 ('R1353N', 'PAL1', PALParameters,
                                  True, [0]),  # PAL1 only
                                 ('R1345D', 'FR5', FRParameters,
                                  False, [0]),  # FR5 only
                             ])
