...
class GenerateTex(ReportRamTask):
    ...
    def run(self):
        tex_template = get_template('pal_stim_report.tex.tpl')
        tex_session_template = get_template('pal_stim_session.tex.tpl')

        tex_biomarker_plot_template = get_template('biomarker_plots.tex.tpl')
...
class GeneratePlots(ReportRamTask):
    ...
    def run(self):
        ...
        for session_summary in session_summary_array:
            panel_plot = PanelPlot(xfigsize=7.5, yfigsize=7.5, i_max=1, j_max=1, title='', wspace=0.3, hspace=0.3, labelsize=20)

            pd1 = PlotData(x=serial_positions, y=session_summary.prob_recall, xlim=(1,6), ylim=(0.0, 1.0), xlabel='Serial position', ylabel='Probability of recall')
            pd1.xlabel_fontsize = 20
            pd1.ylabel_fontsize = 20
            pd2 = PlotData(x=serial_positions, y=session_summary.prob_first_recall, xlim=(0,6), ylim=(0.0, 1.0), xlabel='Serial position\n(b)', ylabel='Probability of first recall')
            ...
            panel_plot = PanelPlot(xfigsize=15, yfigsize=7.5, i_max=1, j_max=2, title='', wspace=4.3, hspace=0.3, labelsize=18)

            pd1 = BarPlotData(x=(0,1,2,3),
                           y=(session_summary.control_mean_prob_diff_all, session_summary.mean_prob_diff_all, session_summary.control_mean_prob_diff_low, session_summary.mean_prob_diff_low),
                           yerr=(session_summary.control_sem_prob_diff_all, session_summary.sem_prob_diff_all, session_summary.control_sem_prob_diff_low, session_summary.sem_prob_diff_low),
                           x_tick_labels=('Control All', 'Stim All', 'Control Low', 'Stim Low'),
            ...
            if ylim > 100.0:
                ylim = 100.0
            pd2 = BarPlotData(x=(0,1), y=session_summary.stim_vs_non_stim_pc_diff_from_mean, ylim=[-ylim,ylim], xlabel='(b)', ylabel='% Recall Difference (Stim-NoStim)', x_tick_labels=['Low', 'High'], xhline_pos=0.0, barcolors=['grey', 'grey'], xlabel_fontsize=18, ylabel_fontsize=18, barwidth=0.5)
            ...
            pd_1 = PlotData(x=stim_x, y=stim_y, ylim=(0,6),
            ...
            pd_2 = PlotData(x=nostim_x , y=nostim_y , ylim=(0,6),
            ...
            for i in xrange(len(session_summary.list_number)-1):
                if session_summary.list_number[i] > session_summary.list_number[i+1]:
                    sep_pos = i+0.5
                    sep_plot_data = PlotData(x=[0],y=[0],levelline=[[sep_pos, sep_pos], [0, 6]], color='white', alpha=0.0)
