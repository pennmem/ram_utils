...
from ram_utils import PlotUtils
...
class GenerateTex(ReportRamTask):
    ...
    def run(self):
        ...
        for session in session_summaries:
            ...
            session_tex += replace_template_to_string(get_template('ps4_session.tex.tpl'),{
                '<SESSION>':session,
        ...
        replace_template(get_template('ps4_report.tex.tpl'),self.get_path_to_resource_in_workspace('reports',report_filename),{
            '<SUBJECT>':subject,
            '<TASK>':task,
            '<DATE>':date.today(),
