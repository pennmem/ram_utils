...
class GenerateTex(ReportRamTask):
    ...
    def get_template(template_name):
        return path.join(path.abspath(path.dirname(__file__)),template_name)

    def run(self):
        task = self.pipeline.task

        tex_template = self.get_template('fr_stim_report.tex.tpl')
        tex_session_template = self.get_template('fr_stim_session.tex.tpl')

        tex_biomarker_plot_template = self.get_template('biomarker_plots.tex.tpl' if task=='RAM_FR4' else 'fr3_plots.tex.tpl')
        tex_itemlevel_comparison_template = self.get_template('itemlevel_comparison.tex.tpl')
