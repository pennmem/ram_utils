...
class ComposeSessionSummary(ReportRamTask):
    ...
    def run(self):
        ...
        for session in sessions:
            ...
            session_data.append(
                [session, session_date, session_length, n_lists, '$%.2f$\\%%' % session_summary.pc_correct_pairs])
        ...
        cumulative_summary.pc_correct_pairs = 100 * cumulative_summary.n_correct_pairs / float(
            cumulative_summary.n_pairs)
        cumulative_summary.wilson1, cumulative_summary.wilson2 = proportion_confint(cumulative_summary.n_correct_pairs,
                                                                                    cumulative_summary.n_pairs,
                                                                                    alpha=0.05, method='wilson')
        cumulative_summary.wilson1 *= 100.0
        cumulative_summary.wilson2 *= 100.0

        cumulative_summary.n_voc_pass = np.sum(events['resp_pass'] == 1)
        ...
        # ------------------------------------- combined classifier - testing on encoding
        combined_cumulative_summary = SessionSummary()
        xval_output_combined = self.get_passed_object('xval_output_combined')
        tested_on_encoding_xval = xval_output_combined['tested_on_encoding']

        pvalue_combined = self.get_passed_object('pvalue_combined')

        combined_cumulative_summary.auc = '%.2f' % (100 * tested_on_encoding_xval.auc)
        combined_cumulative_summary.fpr = tested_on_encoding_xval.fpr
        combined_cumulative_summary.tpr = tested_on_encoding_xval.tpr
        combined_cumulative_summary.pc_diff_from_mean = (
        tested_on_encoding_xval.low_pc_diff_from_mean, tested_on_encoding_xval.mid_pc_diff_from_mean,
        tested_on_encoding_xval.high_pc_diff_from_mean)
        combined_cumulative_summary.perm_AUCs = self.get_passed_object('perm_AUCs_combined')
        combined_cumulative_summary.perm_test_pvalue = (
        '= %.3f' % pvalue_combined) if pvalue_combined >= 0.001 else '\leq 0.001'
        combined_cumulative_summary.jstat_thresh = '%.3f' % tested_on_encoding_xval.jstat_thresh
        combined_cumulative_summary.jstat_percentile = '%.2f' % (100.0 * tested_on_encoding_xval.jstat_quantile)

        self.pass_object('combined_cumulative_summary', combined_cumulative_summary)

        # ------------------------------------- PAL1 classifier - testing on encoding

        pal_cumulative_summary = SessionSummary()
        xval_output_combined = self.get_passed_object('xval_output_combined_pal')
        tested_on_encoding_xval = xval_output_combined['tested_on_encoding']

        pvalue_combined = self.get_passed_object('pvalue_combined_pal')

        pal_cumulative_summary.auc = '%.2f' % (100 * tested_on_encoding_xval.auc)
        pal_cumulative_summary.fpr = tested_on_encoding_xval.fpr
        pal_cumulative_summary.tpr = tested_on_encoding_xval.tpr
        pal_cumulative_summary.pc_diff_from_mean = (
        tested_on_encoding_xval.low_pc_diff_from_mean, tested_on_encoding_xval.mid_pc_diff_from_mean,
        tested_on_encoding_xval.high_pc_diff_from_mean)
        pal_cumulative_summary.perm_AUCs = self.get_passed_object('perm_AUCs_combined_pal')
        pal_cumulative_summary.perm_test_pvalue = (
        '= %.3f' % pvalue_combined) if pvalue_combined >= 0.001 else '\leq 0.001'
        pal_cumulative_summary.jstat_thresh = '%.3f' % tested_on_encoding_xval.jstat_thresh
        pal_cumulative_summary.jstat_percentile = '%.2f' % (100.0 * tested_on_encoding_xval.jstat_quantile)

        self.pass_object('pal_cumulative_summary', pal_cumulative_summary)
        ...
        ttest_table[['Group', 'TagName', 't-stat', 'R', 'G', 'B']].to_csv(
            os.path.join(self.workspace_dir, '_'.join([self.pipeline.subject, task, 'SME_ttest.csv'])), index=False)
