...
class GenerateTex(ReportRamTask):
    ...
    def run(self):
        ...
        tex_template = get_template(task+'_stim_report.tex.tpl')
        tex_session_template = get_template('fr_stim_session.tex.tpl')

        tex_biomarker_plot_template = get_template('biomarker_plots.tex.tpl' if task=='RAM_FR4' else 'fr3_plots.tex.tpl')
        tex_itemlevel_comparison_template = get_template('itemlevel_comparison.tex.tpl')
    ...
    def run(self):
        ...
        for session_summary in session_summary_array:
            ...
            if 'cat' in task:
                ...
                pd = BarPlotData(x=[0, 1], y=[np.mean(irt_within_cat), np.mean(irt_between_cat)],
                ...
                all_rr_hist = np.histogram(all_repetition_ratios,bins=nbins)
