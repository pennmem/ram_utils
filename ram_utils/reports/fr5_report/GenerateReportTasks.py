...
class GenerateTex(ReportRamTask):
    ...
    def get_template(template_name):
        return path.join(path.dirname(path.abspath(__file__)),template_name)
    ...
    def run(self):
        ...
        replace_template(self.get_template('ps4_fr5_report_base.tex.tpl'),self.get_path_to_resource_in_workspace('reports','FR5_report.tex'),
                         {
                             '<SUBJECT>':subject,
            ...
            if ps_events is not None and ps_events.shape:
                for session in np.unique(ps_events.session):
                    if ps_session_summary[session].preferred_location:
                        session_decision = replace_template_to_string(self.get_template('ps_decision.tex.tpl'),
                                                                      {
                    ...
                    ps_session_latex = replace_template_to_string(self.get_template('ps4_session.tex.tpl'),
                                                                  {
                                                                      '<SESSION>':session,
                ...
                ps_latex = replace_template_to_string(self.get_template('PS4_section.tex.tpl'),
                                                      {
                                                          '<SUBJECT>':self.pipeline.subject,
    ...
    def generate_fr5_latex(self):
        ...
        if fr5_events is not None and all(fr5_events.shape):
            ...
            for session_summary in fr5_session_summary:
                ...
                biomarker_tex = replace_template_to_string(self.get_template('biomarker_plots.tex.tpl'),
                ...
                recognition_tex = (replace_template_to_string(self.get_template('recognition.tex.tpl'),
                                                             {
                ...
                session_tex = replace_template_to_string(self.get_template('fr5_session.tex.tpl'),
                             {
                                 '<SESSIONS>':          sessions,
        ...
        fr5_tex = replace_template_to_string(
            self.get_template('FR5_section.tex.tpl'),
            {
                '<SUBJECT>':subject,
