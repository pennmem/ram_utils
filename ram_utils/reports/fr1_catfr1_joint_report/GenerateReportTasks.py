...
class GenerateTex(ReportRamTask):
    ...
    def run(self):
        ...
        tex_combined_template = os.path.join(os.path.dirname(os.path.abspath(__file__)),task + '_combined.tex.tpl')
...
class GeneratePlots(ReportRamTask):
    ...
    def run(self):
        ...
        panel_plot.add_plot_data(0,0,plot_data=hist)
        plot = panel_plot.generate_plot()
        # plot.legend()
        ...
        plot.hist(all_repetition_ratios,bins=int(np.log2(all_repetition_ratios.size)+1),range=[0,1],color='grey')
